...
import type { AllEnvironmentsResult, EnvironmentCheck, EnvironmentResult, NamedCheck, Platform } from './types';
...
function run(cmd: string, opts: ExecSyncOptions = {}): string | null {
  try {
    return (execSync(cmd, { encoding: 'utf8' as const, stdio: 'pipe', timeout: 10000, ...opts }) as string).trim();
  } catch {
    return null;
  }
}
...
/**
 * Run checks for both platforms (used by doctor command).
 */
export function checkAllEnvironments(): AllEnvironmentsResult {
  const android: NamedCheck[] = [];
  android.push({ name: 'Android SDK', ...checkAndroidSDK() });
  android.push({ name: 'Java', ...checkJava() });
  const sdkOk = android.find(c => c.name === 'Android SDK')?.ok;
  if (sdkOk) {
    android.push({ name: 'Emulator', ...checkAndroidEmulator() });
    android.push({ name: 'AVD', ...checkAndroidAVD() });
    android.push({ name: 'Device', ...checkAndroidDevice() });
  }

  const ios: NamedCheck[] = [];
  if (process.platform === 'darwin') {
    ios.push({ name: 'Xcode', ...checkXcode() });
    const xcodeOk = ios.find(c => c.name === 'Xcode')?.ok;
    if (xcodeOk) {
      ios.push({ name: 'Simulator', ...checkSimulator() });
    }
  } else {
    ios.push({ name: 'Platform', ok: false, message: 'iOS requires macOS' });
  }

  const general: NamedCheck[] = [];
  general.push({ name: 'Node.js', ...checkNode() });
  general.push({ name: 'Java', ...checkJava() });

...
describe('logger', () => {
  ...
  it('log.info calls console.log with prefix and message', () => {
    ...
    log.info('hello');
    expect(spy).toHaveBeenCalledWith('[vitest-react-native-runtime]', 'hello');
  });
  ...
  it('log.verbose calls console.log when verbose is on', () => {
    ...
    const spy = vi.spyOn(console, 'log').mockImplementation(() => {});
    log.verbose('debug');
    expect(spy).toHaveBeenCalledWith('[vitest-react-native-runtime]', 'debug');
  });

  it('log.warn calls console.warn with prefix', () => {
    const spy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    log.warn('warning');
    expect(spy).toHaveBeenCalledWith('[vitest-react-native-runtime]', 'warning');
  });

  it('log.error calls console.error with prefix', () => {
    const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
    log.error('err');
    expect(spy).toHaveBeenCalledWith('[vitest-react-native-runtime]', 'err');
  });
});

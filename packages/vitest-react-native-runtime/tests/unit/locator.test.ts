...
import { Locator, createLocatorAPI } from '../../src/runtime/locator';

// A fake resolved element — the locator treats it as opaque
const FAKE_EL = { _type: 'fiber', tag: 1 };
const containerRef = { current: {} } as React.RefObject<any>;
...
describe('createLocatorAPI', () => {
  describe('getByTestId', () => {
    it('returns a Locator that resolves by testID', () => {
      ...
      vi.mocked(readText).mockReturnValue('');
      const api = createLocatorAPI(containerRef);
      const loc = api.getByTestId('btn');
      expect(loc).toBeInstanceOf(Locator);
      expect(loc.exists).toBe(true);
      expect(resolveByTestId).toHaveBeenCalledWith(containerRef, 'btn');
    });
  });
  ...
  describe('getByText', () => {
    it('returns a Locator that resolves by text', () => {
      ...
      expect(loc).toBeInstanceOf(Locator);
      expect(loc.exists).toBe(true);
      expect(resolveByText).toHaveBeenCalledWith(containerRef, 'Submit');
    });
  });
  ...
  describe('getAllByTestId', () => {
    ...
    it('each returned Locator re-resolves by index on access', () => {
      const el0 = { _type: 'fiber', tag: 10 };
      const el1 = { _type: 'fiber', tag: 11 };

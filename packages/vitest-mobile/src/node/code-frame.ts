...
const HIGHLIGHT_COLORS: Record<string, (text: string) => string> = {
  Keyword: pc.magenta,
...
const INTERNAL_LOG_PREFIXES = ['[start]', '[run]', '[runner]', '[expect-setup]', '[vitest-mobile]', '[tree]', '[pool]'];

function isInternalLog(line: string): boolean {
  return INTERNAL_LOG_PREFIXES.some(p => line.includes(p));
}

function highlightCode(file: string, source: string): string {
...
function generateCodeFrame(file: string, line: number, column: number, range = 2): string | null {
  try {

...
const vitestConfig = resolve(repoRoot, 'vitest.config.ts');
...
function runVitest(extraEnv: Record<string, string> = {}): { output: string; exitCode: number } {
  ...
  try {
    const output = execSync(`npx vitest run --project ${platform} --config ${vitestConfig} --reporter=verbose 2>&1`, {
      ...opts,
...
describe('prerequisites', () => {
  ...
  it('test module exists', () => {
    expect(existsSync(resolve(repoRoot, 'test-packages/counter/tests/counter.test.tsx'))).toBe(true);
  });
});
...
describe('smoke: verbose mode (requires VITEST_E2E=1 + emulator)', () => {
  maybeIt(
    ...
    () => {
      ...
      expect(output).toContain('[vitest-mobile]');
